...
const Snake: React.FC = () => {
  ...
  return (
	<Window windowWidth="70vw"
	windowHeight="70vh"
	id={windows.snakeWindow.id}
	title="snake"
	defLeft="15vw">
		
	<div className="gameContainer bg-main h-full w-full flex justify-center items-center" onKeyDown={(e) => changeDirection(e)} tabIndex={0}>
	  <div className="controlPanel flex flex-col items-center justify-between absolute top-[15vh] left-[5vw] gap-8">
	  <div className="scoreBox">
          <h2>Score: {score}</h2>
          <h2>High Score: {highScore}</h2>
        </div>
		<button onClick={play} className="playButton bg-forth border border-third shadow-md px-6 py-2">Play</button>
	  </div>
	  <canvas className="playArea bg-black w-[585px] h-[440px]" ref={canvasRef} width={`${canvasX}px`} height={`${canvasY}px`} />
	  {gameOver && <div className="gameOver fixed top-10rem left-10rem text-third text-3xl">Game Over</div>}
	</div>
  </Window>
